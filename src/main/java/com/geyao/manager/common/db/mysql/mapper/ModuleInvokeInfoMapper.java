...
public interface ModuleInvokeInfoMapper extends BaseMapper<ModuleInvokeInfo> {
    ...
    @Select("select t.*,t1.base_url from t_module_invoke_info t,t_sys_module t1 where t.module_code = t1.module_code and t.invoke_code = #{invokeCode} and t.status = 1 and t1.status=1")
